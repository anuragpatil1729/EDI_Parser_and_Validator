...
const hlLabels: Record<string, string> = { "20": "Billing Provider", "22": "Subscriber", "23": "Patient" };

function issueBelongsToSegment(issue: ValidationIssue, segmentId: string, loopId: string): boolean {
  if (issue.segment !== segmentId) return false;
  if (!issue.loop || issue.loop === "structural") return true;
  return issue.loop === loopId;
}

function getSeverity(issues: ValidationIssue[]): "error" | "warning" | undefined {
...
function LoopCard({ node, depth, issues, onSelectIssue }: { node: LoopNode; depth: number; issues: ValidationIssue[]; onSelectIssue?: (issue: ValidationIssue) => void; }): JSX.Element {
...
export default function SegmentTree({ loops, issues = [], onSelectIssue }: { loops: LoopNode[]; issues?: ValidationIssue[]; onSelectIssue?: (issue: ValidationIssue) => void; }) {
  ...
  return (
    <section className="space-y-3">
      ...
      {orderedLoops.length === 0 ? (
        <div className="rounded-xl border border-slate-200 bg-white p-6 text-center text-slate-500"><FolderTree className="mx-auto mb-2 h-5 w-5 text-slate-400" />No HL loops found in this transaction.</div>
      ) : null}
      {orderedLoops.map((loop) => <LoopCard key={`${loop.loop}-${loop.hl_id ?? "root"}`} node={loop} depth={0} issues={issues} onSelectIssue={onSelectIssue} />)}
    </section>
  );
}
